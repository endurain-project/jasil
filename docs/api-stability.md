...
| `jasil.lifecycle` | `shutdown`, `shutdown_async`, and the order in which they release what JASIL owns. |
...
| `jasil.admin` | The synchronous and `_async` job, event-log, and worker summary reads; synchronous and `_async` dead-letter replay; and the response schemas re-exported alongside them. |
...
| `jasil.jobs.service` | `start_job_worker`, `run_job_worker`, `stop_job_worker`, `stop_job_worker_async`, `schedule_job_maintenance`, `build_runner`. |
